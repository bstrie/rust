...
mod tests {
    ...
    fn check_fail_type(f: Fail_, ft: FailType) {
        match f {
          ArgumentMissing(_) => assert ft == argument_missing_,
          UnrecognizedOption(_) => assert ft == unrecognized_option_,
          OptionMissing(_) => assert ft == option_missing_,
          OptionDuplicated(_) => assert ft == option_duplicated_,
          UnexpectedArgument(_) => assert ft == unexpected_argument_
        }
    }
