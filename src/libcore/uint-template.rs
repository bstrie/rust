...
impl times of iter::times for T {
    #[inline(always)]
    #[doc = " A convenience form for basic iteration. Given a variable `x` of
        any numeric type, the expression `for x.times { /* anything */ }`
        will execute the given function exactly x times. If we assume that
        `x` is a uint, this is functionally equivalent to
        `for uint::range(0, x) {|_i| /* anything */ }`."]
    fn times(it: fn() -> bool) {
        let mut i = self;
        while i > 0 {
            if !it() { break }
