...
pub fn global_memory_watcher_spawner(msg_po: Port<Msg>)
{
    ...
    let mut hm_index: linear::LinearMap<int,
                      @mut linear::LinearMap<libc::uintptr_t, MetricsValue>>
                      = linear::LinearMap();
    ...
    loop {
        match memory_watcher_chan.recv() {
            ReportAllocation(t, s, c, a) => {
                ...
                let Metrics_value = MetricsValue {task_id: *(t),
                                                  size: s,
                                                  td: c };
                let test1: int = (*t);
                ...
                match val1 {
                    Some(T) => {
                        let hm_task_LinearMap:
                            @mut linear::LinearMap<libc::uintptr_t,
                                MetricsValue> = T;
                        hm_task_LinearMap.insert((a as libc::uintptr_t),
                                                 Metrics_value);
                    }
                    None => {
                        let hm_task: @mut linear::LinearMap<libc::uintptr_t,
                            MetricsValue> = @mut linear::LinearMap();
            ...
            ReportDeallocation(t, a) => {
                ...
                match val1 {
                    Some(T) => {
                        let hm_task_deallocate:
                            @mut linear::LinearMap<libc::uintptr_t,
                                MetricsValue> = T;
                        let val2 = hm_task_deallocate.remove(
                            &(a as libc::uintptr_t));
                        if(val2 == true) {
            ...
            TestAllocationAddress(task_id, allocation_address) => {
                ...
                match val1 {
                    Some(T) => {
                        ...
                        let val2 = hm_task_searchvalues.find(
                            &(allocation_address as libc::uintptr_t));
                        match val2 {
                            Some(T) => {
                                println(#fmt(
                                    "Allocation address values found %x",
                                    (allocation_address as uint)));
                            }
                            None => {
                                println("Allocation address value not \
                                        present in metrics");
                            }
                        }
                    }
        ...
        loop {
            match comm::select2(msg_po, po) {
                ...
                Left(ProcessMetricsOfTask(func_process, task_id)) => {
                    ...
                    memory_watcher_po.send(ProcessMetricsOfTask(func_process,
                                                                task_id));
                }
                Left(TestAllocationAddress(task_id, allocation_address)) => {
                    println("In parent test allocation address");
                    memory_watcher_po.send(TestAllocationAddress(task_id,
                        allocation_address));
                }
                Right(_) => {
                    break;
...
fn memory_watcher_start() {
    let global_memory_watcher_ptr =
        rustrt::rust_global_memory_watcher_chan_ptr();
...
fn test_simple() {
    ...
    do spawn {
        ...
        let box1 = @0;
        print_metrics();
        comm_memory_watcher_chan.send(ProcessMetricsOfTask(print_test1,
                                                           tid_secondtask));
        memory_watcher_stop();
    }
    //print_metrics();
